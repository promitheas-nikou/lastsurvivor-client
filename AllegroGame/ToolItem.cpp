...
int ToolItem::GetMaxStackSize() const
{
	return 1;
}
void ToolItem::DrawItemDetailsPane(int x, int y) {
	std::string id = ("#" + GetID());
	...
	al_draw_textf(game_GetFont("default", 20), gameconfig::SOLID_TEXT_COLOR_OK, x + 15, y + 67, 0, "Mining Level: %d", this->GetMiningLevel());
	al_draw_textf(game_GetFont("default", 20), gameconfig::SOLID_TEXT_COLOR_OK, x + 15, y + 89, 0, "Mining Damage: %.1f", this->GetMiningDamage());
	...
	al_draw_multiline_text(game_GetFont("default", 20), al_map_rgba(255, 255, 255, 255), x + 10, y + 111, w, 23, 0, GetDescription().c_str());
}
