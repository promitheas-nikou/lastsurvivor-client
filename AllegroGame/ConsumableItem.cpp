...
void ConsumableItem::DrawItemDetailsPane(int x, int y)
{
    ...
    al_draw_textf(game_GetFont("default", 20), b1 ? gameconfig::SOLID_TEXT_COLOR_OK : gameconfig::SOLID_TEXT_COLOR_ERROR, x + 15, y + 45, 0, "Food: %c%.1f", b1 ? '+' : '-', std::abs(GetFoodBoost()));
    al_draw_textf(game_GetFont("default", 20), b2 ? gameconfig::SOLID_TEXT_COLOR_OK : gameconfig::SOLID_TEXT_COLOR_ERROR, x + 15, y + 67, 0, "Water: %c%.1f", b2 ? '+' : '-', std::abs(GetWaterBoost()));
    al_draw_textf(game_GetFont("default", 20), b3 ? gameconfig::SOLID_TEXT_COLOR_OK : gameconfig::SOLID_TEXT_COLOR_ERROR, x + 15, y + 89, 0, "Health: %c%.1f", b3 ? '+' : '-', std::abs(GetHealthBoost()));
    al_draw_multiline_text(game_GetFont("default", 20), al_map_rgba(255, 255, 255, 255), x + 10, y + 117, w, 23, 0, GetDescription().c_str());
}
